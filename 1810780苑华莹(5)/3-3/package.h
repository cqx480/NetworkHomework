#include<bits/stdc++.h>
#include"bit_cal.h"
#include "common.h"
using namespace std;
const string s="8888";
const string d="8888";

struct package
{
	//首先是UDP的真实首部 
	string srcPort, desPort;
	string len, check_sum;
	string data;

	//下面是TCP新增内容 
	string flag;//16	
	
	//数据包的id 
	string packNum;//32
	package(){}

	package(string s, string d, string fg, string packN, string da)
	{
		srcPort = match(s);
		desPort = match(d);
		check_sum = match("");
		len = ceil(da.size() / 8.0);

		flag = fg;
		
		packNum=match(packN,32);
		data = da;
	}
	bool isACK(int t)
	{
		return flag[ACK]=='1'&&flag[ACK_GROUP]=='0'+t;
	}
	void print()
	{
		cout<<"SrcPort: "<<srcPort<<"\n";
		cout<<"desPort: "<<desPort<<"\n";
		cout<<"len: "<<len<<"\n";
		cout<<"check_sum: "<<check_sum<<"\n";
		cout<<"data: "<<data<<"\n";
		cout<<"flag: "<<flag<<"\n";
		cout<<"packNum: "<<packNum<<"\n";
	}
	
	int ackgroup()
	{
		return flag[ACK_GROUP]-'0';
	}

};

//伪首部 ,在初始化伪首部之前，package必须已经计算完len 
struct fakeHead {
	string srcIP;
	string desIP;
	string zero, protocol, len;

	fakeHead()
	{
		srcIP = match("",32);
		desIP = match("",32);
		zero = match("",8); protocol = match("",8);
		len = match("");
	}
	//传入192.22.22.1这样格式的ip地址
	fakeHead(string s, string d)
	{
		srcIP = decode_ip(s);	//cout<<"srcIP: "<<srcIP<<endl;
		desIP = decode_ip(d);	//cout<<"desIP: "<<desIP<<endl;		
		zero = match("",8); protocol = match("",8);
		len = match("");
	}

	string decode_ip(string s)
	{
		string ans = "";
		int l = 0, r = 0;
		for (int i = 0; i < 4; ++i)
		{
			if (i < 3)r = s.find('.', l);
			else r = s.length();
			string tmp = s.substr(l, r - l);
			tmp = to_bin(tmp);
			ans += match(tmp,8);
			l = r + 1;
		}
		return match(ans,32);
	}
};


fakeHead f("127.0.0.1", "127.0.0.1");

//计算代表长度的16位string 
void get_len(package& p)
{
	int len = ceil(p.data.length() / 8.0);
	string ans = to_string(len);
	ans = match(ans);
	p.len = ans;
}

//计算校验和 
string get_twosum(string s1, string s2)
{
	string ans = "";
	assert(s1.length() == 16 && s2.length() == 16);

	int jin = 0;
	for (int i = 15; i >= 0; --i)
	{
		char cur = (jin + s1[i] - '0' + s2[i] - '0') % 2 + '0';
		ans += cur;
		jin = (jin + s1[i] - '0' + s2[i] - '0') / 2;
	}
	//进位要加回到末位 
	string tmp = "";
	if (jin) {
		for (int i = 15; i >= 0; --i)
		{
			char cur = (jin + ans[i] - '0') % 2 + '0';
			tmp += cur;
			jin = (jin + ans[i] - '0') / 2;
		}
		ans = tmp;
	}
	reverse(ans.begin(), ans.end());
	ans = match(ans);
	return ans;
}
//传入计算好校验和（0)以及长度的package;




string p2s(package p)
{
	string ans = "";
	ans += match(p.srcPort);
	ans += match(p.desPort);
	ans += match(p.len);
	ans += match(p.check_sum);
	ans += p.flag;
	ans += p.packNum;
	ans += p.data;
	return ans;
}

void get_sum(package& p)
{
	string s=p2s(p);

	//为了更快计算，采用强制转化成int*的方式 
	short int*s_16=(short int*)&s[0];
	unsigned short int cur_ans=0;
	for(int i=0;i<s.size()/2;++i)
	{
		cur_ans+=s_16[i];
	}
	
	string ans=match(to_string(cur_ans));

	
	//求反码 
	for (int i = 0; i < 16; ++i)ans[i] = '1' - ans[i] + '0';
	
	p.check_sum = ans;
}
//伪首部只是用来计算校验和，并不打包进package 
string encode(package& p)
{
	string ans = "";
	ans += match(p.srcPort);
	ans += match(p.desPort);

	//计算长度 
	get_len(p);
	ans += match(p.len);

	//计算校验和 
	get_sum(p);
	ans += match(p.check_sum);


	ans += p.flag;
	ans += p.packNum;
	ans += p.data;
	return ans;
}

void decode(string s, package& p)
{
	p.srcPort = s.substr(0, 16);
	p.desPort = s.substr(16, 16);
	p.len = s.substr(32, 16);
	p.check_sum = s.substr(48, 16);
	p.flag = s.substr(64, 16);
	p.packNum = s.substr(80, 32);
	p.data = s.substr(112);
}


bool check_lose(package p)
{
	//保存旧的校验和 
	string olds = p.check_sum;
	p.check_sum = match("");

	get_sum(p);
	string news = p.check_sum;
	
	if(news!=olds){
		cout<<"new sum: "<<news<<endl;
		cout<<"old sum: "<<olds<<endl;
	}
	return news == olds;
}








